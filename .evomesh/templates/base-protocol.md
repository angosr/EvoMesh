...
2. **演进质量门**：提议变更前必须回答：(a) 解决什么具体问题？引用 metrics 或事件 (b) 行为如何变化？仅措辞变更 = 跳过 (c) 如何衡量效果？"无需变更"是合格的审查结果
3. P0/P1 改动提议发 lead inbox；P2 由 lead 授权的角色可直接实施
4. 记录到 `evolution.log`
5. 🔒 标记的规则不可通过自我演进修改（仅用户/lead 可改）
