...
Phase 3 已完成，进入 Phase 4 打磨阶段。server/index.ts 拆分重构已完成。CI (GitHub Actions) 已上线，30 个单元测试全通过。

## 角色状态

| 角色 | 状态 | 当前工作 |
|------|------|----------|
| lead | 运行中 | Loop #9 — 验证 index.ts 拆分，更新文档 |
| executor | 运行中 | Loop #16 — 大方向审查，报告文档过时 |
...
- WebSocket 断线重连
- server 模块化拆分 (index + routes + terminal)
...
| ~~P2~~ | ~~expandHome fallback 错误~~ | ✅ 已修复 (executor Loop #10) |
| ~~P2~~ | ~~库函数调用 process.exit()~~ | ✅ 已修复 (executor Loop #10) |
| P2 | 单元测试覆盖不足 | 部分完成（30 个单元测试，server 路由未覆盖） |
